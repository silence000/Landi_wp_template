...
{
    $avatar = preg_replace('/.*\/avatar\/(.*)\?s=([\d]+)&.*/', '<img src="/wp-content/themes/landinetwork/img/admin-logo.png" class="avatar avatar-$2" height="$2" width="$2">', $avatar);
    return $avatar;
}
add_filter('get_avatar', 'wpyou_get_ssl_avatar');


// 7、WordPress 后台管理菜单名称重命名的方法
...
// 8、面包屑插件  调用插件方法 php if(function_exists('cmp_breadcrumbs')) cmp_breadcrumbs(); 前后加php标记符号
function cmp_breadcrumbs() {
	...
	if ( !is_home() && !is_front_page() || is_paged() ) {
		...
		if ( is_category() ) { // 分类 存档
			...
			echo $before ;
			printf( __( '搜索关键词: %s', 'cmp' ),  get_search_query() );
			echo  $after;
		} elseif ( is_tag() ) { //标签 存档
			echo $before ;
